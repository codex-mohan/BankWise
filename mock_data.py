...
class MockDataGenerator:
    ...
    def _generate_accounts(self) -> List[Dict]:
        ...
        for i in range(100):
            ...
            account = {
                ...
                'linked_cards': [f"****{random.randint(1000, 9999)}" for _ in range(random.randint(1, 3))]
            }
        ...
        for account in self.accounts:
            for _ in range(random.randint(1, 3)):
                ...
                card = {
                    ...
                    'card_status': random.choice(['ACTIVE', 'BLOCKED', 'EXPIRED', 'LOST']),
                    ...
                    'issue_date': (datetime.now() - timedelta(days=random.randint(30, 1095))).isoformat(),
    ...
    def _generate_transactions(self) -> List[Dict]:
        ...
        transactions = []
        transaction_types = ['DEPOSIT', 'WITHDRAWAL', 'TRANSFER', 'PURCHASE', 'CASHBACK', 'INTEREST']
        merchants = ['Amazon', 'Flipkart', 'Walmart', 'Target', 'Starbucks', 'McDonalds', 'Uber', 'Swiggy']
        
        for account in self.accounts:
            # Generate 50-200 transactions per account
            num_transactions = random.randint(50, 200)
            for i in range(num_transactions):
                transaction_date = datetime.now() - timedelta(days=random.randint(1, 730))
                ...
                balance_after = account['balance'] - amount if transaction_type in ['WITHDRAWAL', 'TRANSFER', 'PURCHASE'] else account['balance'] + amount
                ...
                transaction = {
                    ...
                    'merchant_id': random.choice(merchants) if transaction_type == 'PURCHASE' else None,
        ...
        for city in cities:
            for i in range(random.randint(3, 8)):
                branch = {
                    ...
                    'branch_type': random.choice(['FULL_SERVICE', 'ATM_ONLY', 'BUSINESS_CENTER']),
                    'facilities': random.choice(['ATM', 'LOCKER', 'FOREX', 'NET_BANKING', 'MOBILE_BANKING']),
                    'manager_name': fake.name(),
                    'established_date': (datetime.now() - timedelta(days=random.randint(3650, 18250))).isoformat()
                }
    ...
    def _generate_atms(self) -> List[Dict]:
        ...
        atms = []
        banks = ['Bank of Baroda', 'State Bank of India', 'HDFC Bank', 'ICICI Bank', 'Punjab National Bank']
        cities = ['Mumbai', 'Delhi', 'Bangalore', 'Hyderabad', 'Chennai', 'Kolkata', 'Pune', 'Ahmedabad', 'Jaipur', 'Lucknow']
        ...
        for city in cities:
            for i in range(random.randint(10, 25)):
                atm = {
                    ...
                    'facilities': random.choice(['CASH_DEPOSIT', 'CASH_WITHDRAWAL', 'BALANCE_ENQUIRY', 'MINI_STATEMENT', 'CARD_RENEWAL']),
                    'last_maintenance': (datetime.now() - timedelta(days=random.randint(1, 90))).isoformat(),
                    'status': random.choice(['ACTIVE', 'OUT_OF_SERVICE', 'MAINTENANCE'])
                }
                atms.append(atm)
    ...
    def _generate_complaints(self) -> List[Dict]:
        ...
        complaints = []
        categories = ['ACCOUNT', 'CARD', 'TRANSACTION', 'ATM', 'BRANCH', 'LOAN', 'FD', 'NET_BANKING', 'MOBILE_BANKING', 'OTHER']
        ...
        for i in range(200):
            ...
            complaint = {
                ...
                'assigned_agent': f"AGENT{random.randint(100, 999)}" if random.random() < 0.8 else None,
                'resolution_notes': fake.text(max_nb_chars=100) if resolved_date else None,
                'customer_satisfaction': random.randint(1, 5) if resolved_date else None
            }
    ...
    def _generate_disputes(self) -> List[Dict]:
        ...
        disputes = []
        dispute_types = ['FRAUD', 'UNAUTHORIZED', 'BILLING_ERROR', 'SERVICE_CHARGE', 'OTHER']
        ...
        for i in range(100):
            ...
            dispute = {
                ...
                'assigned_officer': f"OFFICER{random.randint(100, 999)}" if random.random() < 0.7 else None,
                'resolution_notes': fake.text(max_nb_chars=100) if resolved_date else None,
    ...
    def _generate_loans(self) -> List[Dict]:
        ...
        loans = []
        loan_types = ['HOME_LOAN', 'PERSONAL_LOAN', 'CAR_LOAN', 'EDUCATION_LOAN', 'GOLD_LOAN', 'BUSINESS_LOAN']
        statuses = ['DISBURSED', 'ACTIVE', 'COMPLETED', 'DEFAULT', 'CLOSED']
        
        for i in range(150):
            disbursement_date = datetime.now() - timedelta(days=random.randint(30, 3650))
            ...
            loan = {
                ...
                'remaining_tenure': tenure_months - random.randint(0, tenure_months - 1),
                'status': random.choice(statuses),
                'collateral_details': fake.text(max_nb_chars=100) if random.random() < 0.7 else None,
                'processing_fee': round(random.uniform(1000, 50000), 2),
                'insurance_details': fake.text(max_nb_chars=50) if random.random() < 0.6 else None
            }
            loans.append(loan)
        ...
        for tenure in tenures:
            for customer_type in customer_types:
                ...
                fd_rate = {
                    ...
                    'special_features': random.choice(['TAX_SAVING', 'MONTHLY_PAYOUT', 'QUARTERLY_PAYOUT', 'CUMULATIVE'])
                }
    ...
    def get_transactions_by_account(self, account_number: str, limit: int = 5) -> List[Dict]:
        """Get transactions for an account"""
        account_transactions = [t for t in self.transactions if t['account_number'] == account_number]
